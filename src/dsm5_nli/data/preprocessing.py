...
def load_and_preprocess_data(config) -> pd.DataFrame:
    ...
    # Load posts
    console.print(f"[yellow]Loading posts from:[/yellow] {config.data.posts_path}")
    posts_df = pd.read_csv(config.data.posts_path)
    console.print(f"[green]✓[/green] Loaded {len(posts_df)} posts")

    # Load annotations
    console.print(f"[yellow]Loading annotations from:[/yellow] {config.data.annotations_path}")
    annotations_df = pd.read_csv(config.data.annotations_path)
    console.print(f"[green]✓[/green] Loaded {len(annotations_df)} annotations")

    # Load DSM-5 criteria
    console.print(f"[yellow]Loading DSM-5 criteria from:[/yellow] {config.data.criteria_path}")
    with open(config.data.criteria_path, 'r') as f:
    ...
    # Add criterion text
    pairs_df["criterion"] = pairs_df["DSM5_symptom"].map(criteria_dict)

    # Create binary label (1 if present, 0 if absent)
    pairs_df["label"] = (pairs_df["status"] == 1).astype(int)

    # Select and rename columns
    pairs_df = pairs_df[["post_id", "text", "DSM5_symptom", "criterion", "label"]]
    pairs_df = pairs_df.rename(columns={"text": "post", "DSM5_symptom": "criterion_id"})
