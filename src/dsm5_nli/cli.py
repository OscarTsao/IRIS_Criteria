...
def run_single_fold(
    ...
    console.print(f"Model parameters: {model.get_num_trainable_params():,} trainable / {model.get_num_total_params():,} total")
...
def run_kfold_training(config: DictConfig):
    ...
    enable_deterministic(config.reproducibility.tf32)
    ...
    console.print(f"Mean F1: {mean_metrics['mean_f1']:.4f} ± {np.std([r['val_f1'] for r in fold_results]):.4f}\n")
